...
- 뉴스 Tool 결과는 UI가 별도 뉴스 카드로 보여준다. 기사별 내용을 장문으로 반복하지 말고
  먼저 핵심 판단을 1~2문장으로 요약한 뒤, 필요하면 최대 5개의 짧은 목록만 쓴다.
- Tool의 구조화 숫자·시계열·공시·리포트 결과는 UI가 표와 차트로 보여주므로 모든 값을
  줄글로 복제하지 않는다. 사용자가 이해할 핵심 의미와 주의점만 간결하게 설명한다.
