...
> Ext A~D는 Phase 6과 Phase 7 사이 문서에 정의된 **향후 확장**이며 미구현이다.
> Agentic·MCP·A2A는 아직 완료 기능이 아니다.
...
# Extension A~D. Agentic·MCP·A2A 확장 (향후 계획)

> **주의**: Extension A~D는 Phase 5(리포트 RAG)와 Phase 6(주가)이 완료된 뒤 진행하는
...
## Extension D. A2A PoC (Fact Agent · Research Agent 분리)

### 목표
필요성·일정이 확인되면 Fact Agent(정확 숫자·공시)와 Research Agent(뉴스·리포트 설명)를
분리한 A2A PoC를 구현한다. PoC 단계로만 두고 프로덕션 경로에는 넣지 않는다.

### 작업 체크리스트
- [ ] A2A 도입 필요성·일정 확인(안 하면 진행하지 않음)
- [ ] Fact Agent / Research Agent 역할 경계 정의
- [ ] 두 Agent 간 메시지 규약 정의
- [ ] PoC 격리(프로덕션 라우터에 미연결)
- [ ] PoC 결과·한계 보고

### 최소 통과 조건
- PoC가 프로덕션 답변 경로에 영향 없음
- 필요성 미확인 시 미진행 결정을 문서에 기록
