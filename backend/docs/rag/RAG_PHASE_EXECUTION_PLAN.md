...
전체 pytest 288 통과, ruff·format 통과.

> **운영 배포 검증(2026-07-25, PR #41 머지커밋 1df5d04)**: 운영 컨테이너=1df5d04,
> healthy·/docs 200·AGENT_ENABLED=true·TOSS 키 설정(값 미노출)·VM IP 34.64.197.122
> 토스 허용목록 등록 확인. 운영 /api/qa 10문 전부 agent=true: 현재가·기간수익률·
> "목표주가 말고"(리포트 미호출)·비교(둘 다)·사건 전후·휴장일 스냅·없는종목(대체 없음)·
> no_data·캐시 히트 모두 정상. 계산 Exact Match(1m -25.63%, 5/4 +24.43%, 5/3 +28.31%).
> SSE agent_start→tool_start→tool_end→sources→delta→done, error 0·인자/추론 미노출.
> 기존 5기능(용어·재무·뉴스제외·리포트·no_data) 회귀 정상. 토스 429·비밀키 로그 노출 0.
> 관찰(수정 없음): "목표주가 말고"는 calculate_event_return 선택(백엔드 계산·리포트 미호출로 조건 충족),
> 없는종목 답변에 검증기 재무숫자 경고 오탐(답변 영향 없음).
...
## Phase 8 2단계 — 라벨 검토 (2026-07-26)

`needs_manual_review` 65건(뉴스 25·리포트 20·복합 20)을 DB 원본과 대조해 확정.
정답 식별자 90개(뉴스 청크 28·리포트 청크 62) 생성, 검증 24/24 통과. 운영 코드 무수정.
상세: `docs/rag/phase_8/PHASE_8_LABEL_REVIEW.md`

## Phase 8 3단계 — 개발셋 baseline (2026-07-26)

개발셋 120문항 1회 실행. **홀드아웃 미실행. 운영 코드·프롬프트·검색 설정 무수정.**

- 120/120 완료, 외부 환경 오류 0건, 524초, $0.546(질문당 $0.00455)
- 필수 Tool 호출률 0.945 / Tool 인자 정확도 0.873 / 타 종목 혼입 0
- Citation Precision 1.0 / 존재하지 않는 출처 0 / 허위 답변 0 / 과도한 단정 0
- P50 3.7s / P95 9.1s
- 무결점 42문항, 치명적 실패 54건(중복 계수)
- 최대 실패: 검증기가 근거 있는 답변 문장 제거 22건, Tool 열거형 인자 오류 9건
- 라벨·채점기 문제 19건 내외 확인 → 검색 Recall 0.333 은 실제보다 낮게 측정된 값
- **홀드아웃 실행은 보류**(치명적 실패·라벨 보정 선행 필요)

상세·수정 제안: `docs/rag/phase_8/PHASE_8_DEV_BASELINE.md`

미수행: 운영 코드 수정·홀드아웃 실행·reranker·사람 평가 수행·운영 배포·Phase 9

## Phase 8 4단계 — 1차 교정 (2026-07-26)

검색 튜닝 전에 baseline 을 왜곡하던 검증기 오탐·Tool 입력 계약·평가기 오류를 수정.
**홀드아웃 미실행, 개발셋 120문항 재실행도 안 함**(관련 65문항만 targeted regression).

근본 원인은 대부분 **유니코드 정규화 불일치**였다 — DB 의 증권사명이 NFD, 모델 답변이
NFC 라 같은 이름도 불일치로 판정돼 검증기가 근거 있는 목표주가 답변을 삭제했다.
같은 결함이 리포트 broker 검색 필터에도 있었다.

- 검증기: NFC 정규화 적용, 사실 주장이 아닌 숫자(예시·페이지·종목코드) 검증 제외
- 공시 Tool: `event_types` 를 DB 실제 값 9개 Literal 로 강제 + 한국어 대응표 명시,
  허용되지 않은 값은 no_data 가 아니라 입력 오류로 구분
- 재무 Tool·프롬프트: "단독"은 3개월치이지 별도재무제표(OFS)가 아님을 명문화
- 평가기: 검색 지표를 document_retrieval / structured_lookup 으로 분리,
  검증기가 답을 지운 경우를 Retriever 실패에서 제외, 치명도 중복/고유 계수 분리
- 라벨: `fin-03` 1건만 모호 질문으로 재분류(두 해석 모두 DB 실재, 근거 기록)

targeted regression 65문항: 답변 삭제 22→3(남은 3건은 정탐), no_data 13→0,
재무 정확 일치 11→13. 전체 457 passed / 1 failed(기존 .env 의존, 무관).

남은 실제 검색 문제: 뉴스 색인 중복(비현행 활성 청크 941건). 다음 라운드 대상.

상세: `docs/rag/phase_8/PHASE_8_CORRECTION_ROUND1.md`,
문제·해결 정리: `docs/rag/phase_8/PHASE_8_ISSUES_AND_FIXES.md`

미수행: 검색 튜닝·reranker·뉴스 재색인·홀드아웃 실행·운영 배포·Phase 9

## Phase 8 5단계 — 개발셋 baseline round2 (2026-07-26)

1차 교정(PR #57) 반영 후 개발셋 120문항 재실행 + round1 대비 비교. 같은 채점 기준으로
비교하려고 round1 원시 실행 결과를 현재 채점기로 재채점한 대조군을 만들어 사용했다.
**홀드아웃 미실행, 운영 코드 무수정.**

- 120/120 완료, 외부 환경 오류 0건, 510초, $0.602
- 무결점 문항 42→**73**, 치명적 오류(중복계수) 54→**12**(고유 10)
- Tool 인자 정확도 0.873→**0.953**, 구조화 조회 성공률 0.533→**0.956**,
  숫자 Exact Match 0.789→**0.947**
- 검증기 오탐(답변 삭제) 22→2건(남은 2건은 실제 환각을 잡은 정탐으로 확인)
- 공시 입력 오류 9→0, 재무 `no_data` 오판(단독/별도 혼동) 2→0
- 문서 검색(뉴스·리포트) Recall@K 는 round1·round2 모두 0.0 — 회귀가 아니라
  검증기 오탐을 걷어내고 남은 순수 검색 실패 값
- **뉴스 비현행 청크 941건과 검색 실패의 실제 연결 확인**: 뉴스 Retriever 실패
  15건 중 **14건(93%)**이 비현행 중복 문서와 실제로 연결됨(정리는 하지 않음)
- 남은 실패를 9계층으로 분류: 뉴스 Retriever 14 · Agent Tool 선택 13 ·
  평가기·라벨 16(대부분 정탐) · Tool 인자 7 · Validator 7(대부분 정탐) 등

상세: `docs/rag/phase_8/PHASE_8_DEV_BASELINE_ROUND2.md`

다음 우선순위(제안, 미실행): ① 뉴스 재색인/중복 정리 ② Agent Tool 선택 프롬프트
보강 ③ 평가기·라벨 2차 정리

미수행: 뉴스 재색인·Agent 코드 수정·검색 튜닝·홀드아웃 실행·운영 배포·Phase 9

> **정정(6단계에서 반증됨)**: 위 "비현행 청크 93% 연결"은 상관관계를 인과로 읽은 오진이다.
> 검색 RPC 는 원래 `is_current=true` 를 강제하므로 비현행 청크는 검색되지 않는다.
> 진짜 원인은 종료일 경계에서 당일 뉴스가 잘린 것이었다. 아래 6단계 참조.

---

## Phase 8 6단계 — 2차 교정 (2026-07-26)

round2 실패의 공통 원인 수정. **개발셋 120문항 전체·홀드아웃 미실행**, targeted regression
30문항까지만 수행.

- **round2 진단 정정**: 검색 RPC(`rag_search_hybrid`·`rag_search_semantic`) 정의를 직접
  확인한 결과 두 함수 모두 `c.is_active AND d.is_current` 를 강제한다 → 비현행 청크는
  애초에 검색되지 않는다. 실패 15건의 gold 클러스터도 전부 현행 문서가 1개뿐이었다.
- **진짜 원인**: `relative_period="recent"` 가 주는 종료일(`2026-07-26`)이 timestamptz 로
  그 날 00:00 으로 해석돼 **당일 뉴스가 전부 잘렸다**. 뉴스 청크는 99%가 실제 발행 시각을
  가져서(리포트는 전부 자정) 후보가 296→218건(26% 소실)으로 줄었고, gold 사건은 전부
  당일이라 구조적으로 검색 불가였다. `list_recent_news` 에는 이미 있던 보정이
  하이브리드 경로에만 빠져 있었다.
- 수정: `app/rag/retrieval.py` 에 `_inclusive_end()` 공용 헬퍼(임베딩·RRF·top-k 무변경)
- 뉴스 정답 문서 적중 **0 → 9/14**, 필수 Tool 누락 **6→0**, Tool 인자 오류 **6→0**,
  금지 Tool **1→0**, 타 종목 혼입 0 유지, 검증기 안전장치 3종 수치 불변
- 프롬프트 최소 수정: 복합 질문 하위 요청 분해(원인·배경 → `search_news`),
  종목 지정 양방향 규칙(문맥 있으면 되묻지 않기 / 없으면 임의 선택 금지)
- 비현행 활성 청크 **945 → 0** (soft 처리, hard delete 없음, 현행 3,494건·클러스터
  7,229건 불변, 2회차 실행 대상 0건으로 idempotent 실증)
- 평가기 중복 계수 14건 제거 → 실패 47→**35**, 무결점 73→**85** (제품 개선이 아니라
  잘못 센 값의 정정)
- 테스트 468 passed / 1 failed(기존), 신규 11건

상세: `docs/rag/phase_8/PHASE_8_CORRECTION_ROUND2.md`

미수행(의도적): 리포트·뉴스 **의미 검색 품질** 개선(`report-06/08`, `news-04/09/14/15/19`) —
임베딩·RRF·reranker 변경 금지 범위. 홀드아웃 실행·운영 배포·Phase 9.

---

## Phase 8 7단계 — 개발셋 baseline round3 (2026-07-26)

2차 교정(PR #59) 반영 후 개발셋 120문항 전체 재실행. **평가·분석만 수행, 코드·프롬프트·
Tool description·검색 설정·평가 라벨 무수정. 홀드아웃 미실행.**

- 120/120 완료. 무결점 42(baseline)→85(round2 재집계)→**96**(round3),
  고유 실패 78→35→**24**
- 필수 Tool 호출률 0.945→0.953→**1.0**, Tool 인자 정확도 0.873→0.953→**1.0**,
  복합 질문 완료율 0.833→0.833→**1.0**, 숫자 Exact Match 0.789→0.947→**1.0**
- **round2 원인 진단 재검증**: 비현행 활성 청크가 945→0(PR #59) 이후 라이브 파이프라인이
  계속 돌며 9건 다시 생겼으나, 이번 라운드 실패 뉴스 7건의 gold 문서는 전부 정상 현행 —
  검색 실패는 색인 문제가 아니라 순수 의미 검색 순위 문제로 확인
- 종료 기준 8개 중 7개 충족. "제외 조건 준수율"만 수치상 93.5%로 미달이나, 4건 전부
  원문 대조 결과 **그레이더 부정 마커 목록의 한계로 인한 오탐**(실제 위반 0건)으로 확인
- 남은 24건을 10계층으로 분류: 뉴스 Retriever 7 · 리포트 Retriever 4(검색 순위 문제) ·
  Agent Tool 선택 5(답변은 정확) · Generation 4(전부 오탐) · Validator 5(전부 정탐) ·
  평가기·라벨 1(`search_disclosures` 비결정성 의심)
- 리포트 문항 15개 전수 확인 — 전부 증권사·날짜 명시, "명확화 필요" 유형은 devset에 없음

상세: `docs/rag/phase_8/PHASE_8_DEV_BASELINE_ROUND3.md`

다음 최소 수정 후보(제안, 미실행): ① 그레이더 부정 마커("아님"·"~외") 보강
② forbidden_claims 라벨 재검토(disc-13) ③ search_disclosures 비결정성 원인 조사

미수행: 코드 수정 전체(이번 라운드 금지 범위), 뉴스·리포트 의미 검색 개선,
홀드아웃 실행·운영 배포·Phase 9.
