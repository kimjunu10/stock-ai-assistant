...
> "향후 확장 구조(Tool Registry / 제한형 Agentic / MCP)"는 **아직 미구현**이다.
> Agentic·MCP를 완료된 기능처럼 서술하거나 사용하지 않는다. **A2A는 이번 프로젝트 범위에서 제외**한다.

### 현재 구조 (구현 완료)

- LangChain·LangGraph 없이 **직접 구현한 결정론적 하이브리드 RAG**.
- 질문 계획(§9)은 규칙 기반 라우터(QueryPlan)로, LLM 호출 없이 신호어로 필요한 작업을 결정한다.
- 필요한 조회(재무 SQL·용어·뉴스 하이브리드 검색)를 병렬 실행 후 결과를 통합한다.
- 반복 검색 루프·에이전트 간 협상·동적 도구 선택 LLM 호출이 없다(결정론적).
- **QA 라이브 경로**: `/qa`·`/qa/stream` 단일 진입점이 QueryPlan 판정으로
  순수 뉴스→하이브리드 검색, 숫자→재무 SQL, 용어→lookup_term, 혼합→결합을 처리한다.
  실제 숫자는 SQL 값을 그대로 문맥에 넣어 LLM이 재계산·변경하지 못하게 하고,
  value_kind(실제/전망)·단위·기간·출처를 보존한다.
  (2026-07-24 통합 전까지 이 결정론적 경로는 구현·테스트만 되고 API에는 미연결이었으며,
  라이브 `/qa`는 뉴스 전용 검색만 서빙했다. 통합으로 결정론적 QA 라이브 경로가 완료되었다.)
...
3. **Extension C — MCP 서버 공개 (선택)**
   - 동일한 Tool을 MCP 서버로 노출해 외부(또는 다른 런타임)에서 재사용 가능하게 한다.
   - 내부 구현(직접 함수 호출)과 MCP 노출은 같은 Tool 정의를 공유한다.
   - **Tool·Agentic이 안정화되고 일정·시연 가치가 있을 때만** 선택적으로 진행한다.

4. **A2A — 이번 프로젝트 범위에서 제외**
   - 독립 Agent 간 작업 위임(A2A)은 구현하지 않는다. 현재는 단일 백엔드에서 공통 read-only
     Tool을 호출하는 구조가 더 단순·안정적이라고 판단했다. 적용하지 않은 사유만 기록한다.

각 확장의 실행 단계는 `RAG_PHASE_EXECUTION_PLAN.md`의 Extension A~C(및 A2A 제외 사유)를 따른다.
...
### 문서 검색(need_documents) 판정

문서(뉴스 하이브리드) 검색은 **설명이 필요한 질문에만** 켠다. 사실만 묻는 질문에서
불필요한 뉴스 검색·임베딩 API 호출을 하지 않도록 의도 신호로 구분한다.

```text
설명 신호(왜·영향·전망 등) 또는 정정 신호 있음 → 문서 검색 켬
순수 숫자 질문(숫자 신호만, 설명 없음)        → 문서 검색 끔 (재무 SQL 만)
순수 용어 질문(용어 신호만, 설명 없음)        → 문서 검색 끔 (용어 조회만)
사실 신호(숫자/용어) 없는 자연어 질문         → 문서 검색 켬 (기본 뉴스 검색)
```

예시:
- "영업이익 얼마?" → 재무 SQL 만
- "영업이익 얼마고 왜 변했어?" → 재무 SQL + 문서 검색
- "PER이 뭐야?" → 용어 조회만
- "최근 뉴스 알려줘" → 문서 검색

특정 질문 문장을 하드코딩하지 않고 위 신호 조합으로만 판정한다.
