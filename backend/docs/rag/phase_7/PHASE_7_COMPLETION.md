...
## 15. 운영 배포 smoke (2026-07-26)

PR #47 머지(`848eeed`) 후 운영 컨테이너가 해당 커밋 이미지로 재기동된 것을 확인하고,
운영 API에 최소 smoke만 실행했다. 코드 수정·전체 재평가·전체 문서 재독은 하지 않았다.

- 운영 배포 커밋: `848eeed`(컨테이너 이미지 태그와 일치, `docker inspect` 확인)
- health: `GET /docs` 200, 컨테이너 상태 `healthy`
- Agent 상태: `AGENT_ENABLED=true`, `APP_ENV=production`, `/api/qa`·`/api/qa/stream` 정상 노출

### 질문별 통과 여부

| # | 시나리오 | 결과 |
|---|---|---|
| A | 현재 주가·등락률 | ✅ `get_stock_prices`, 2026-07-24 252,500원/-7.51%, 출처 1건 |
| B | 여러 사건 후속(문맥 없음) | ✅ Tool 호출 0, 사건 임의 선택 없음, 기간 대체 없음, 명확화 요청만 |
| C | 특정 과거 사건 이후(`news_cluster:3797`) | ✅ `calculate_event_return`, baseline 07-20 251,500원 → 1일 +4.77%(07-21)·3일 +8.55%(07-23), 종료 게이트와 동일 |
| D | 발표 후 데이터 없음(2026-07-25 사건) | ✅ `calculate_event_return:no_data`, 기간 대체 없음, 시각화 없음 |
| E | 존재하지 않는 종목 999999 | ✅ 주가 `error` / 뉴스 `no_data` 구분, 혼입 없음 |

사건 임의 선택 0건, 다른 기간 대체 0건, 근거 없는 허위 숫자 0건, 출처 누락 0건.

### `/qa/stream` 결과
시나리오 C를 SSE로 1회 추가 실행: `agent_start → tool_start → tool_end → sources → delta → done`
정상 종료, error 이벤트 없음. `/qa`와 사건 식별자·발표일·baseline·1·3거래일 수익률·출처가
완전히 일치. 내부 추론·전체 Tool 입력·비밀값 노출 없음.

### 실패·재시도
실패 시나리오 없음. 재시도 없음.

### 판정
**Phase 7 최종 완료**. 운영 배포 커밋이 PR #47 반영 커밋과 일치하고, 종료 게이트 핵심
시나리오가 운영 환경에서도 동일하게 재현됐다.
