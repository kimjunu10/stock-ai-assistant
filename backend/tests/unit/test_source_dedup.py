...
def _chunk(cid, doc, evt, chash, order=0) -> RetrievedChunk:
    return RetrievedChunk(
        chunk_id=cid, document_id=doc, content=f"c{cid}", value_kind="news_interpretation",
        stock_code="005930", source_type="news_event", published_at="2026-07-22",
        source_pk=evt, title="t", publisher="p", source_url="u", similarity=0.5,
        chunk_order=order, content_hash=chash,
    )
