...
SYSTEM_PROMPT = (
    ...
    "1~2문장, 140자 이내로 쓴다. 첫 문장은 반드시 '쉽게 말해, '로 시작하고 회사에 무슨 일이 생겼는지와 "
    "그 일이 회사에 어떤 의미인지 일상적인 말로 바로 설명한다. 배경과 수치를 나열하지 말고 가장 중요한 "
...
COMPACT_RETRY_SYSTEM_PROMPT = (
    ...
    "title은 90자 이내, easy_explanation은 '쉽게 말해, '로 시작하는 1~2문장·140자 이내, factual_body는 "
...
EASY_EXPLAIN_SYSTEM_PROMPT = (
    ...
    "선택 문구의 뜻을 먼저 일상적인 말로 한 문장에 설명하고, 필요할 때만 이 뉴스에서 왜 "
    "언급됐는지를 한두 문장 덧붙인다. 전체는 최대 3문장, 180자 이내로 쓴다. 서로 다른 "
    "내용은 빈 줄로 나눠 최대 2개 짧은 문단으로 만든다. 기사 전체를 요약하거나 문맥의 수치와 "
    "사례를 나열하지 않는다. 전문용어를 새로 쓰지 말고 '~해요' 말투를 사용한다. "
