...
# 명확한 오피니언(사설·칼럼·기고·기자수첩·데스크칼럼·논설·시론·만평·요일광장 등) 표지.
# 단순 문자열 포함으로 정상 사건 기사(예: "피카딜리 광장…")를 오분류하지 않도록,
# 광장은 '요일광장' 또는 대괄호로 닫히는 경우(광장])만, 그 외 표지는 명시어로 좁힌다.
# 실제 평가 데이터에서 확인된 매칭 예:
#   [글로벌이코노믹 사설] … / [정동칼럼]아! 삼전닉스 … / [화요광장] 레버리지 … /
#   [만화경] 레버리지 ETF 20년사 / [데스크칼럼] … / [기자수첩]몸통 흔드는 …
OPINION_PATTERN = (
    r"사설|칼럼|기고|기자\s*수첩|데스크|논설|시론|만평|만화경|"
    r"[월화수목금]요\s*광장|광장\]|"
    r"\[사설\]|\[시론\]|\[기고\]|\[초점\]|\[진단\]|\[시선\]|\[돋보기\]|\[뷰\]"
)
_OPINION_RE = re.compile(OPINION_PATTERN)


def is_opinion(title: str, description: str = "") -> bool:
    """명확한 오피니언 기사 여부(제목 표지 기반). 하나의 구체적 사건 보도가 아니라
    의견·논평 글이므로 company 동일사건 클러스터링 입력에서 제외하기 위한 gate.

    원문 섹션/카테고리가 저장되지 않아 제목 텍스트로만 판정한다.
    오탐을 줄이려고 표지어를 좁게 정의한다(예: '광장'은 요일광장/'광장]'만)."""

    return bool(_OPINION_RE.search(title or ""))
