...
ASSIGN_V2_PROMPT_VERSION = "same_event_sig_v2"

SYSTEM_PROMPT_V2 = (
    "너는 한국어 금융 뉴스의 '동일 사건' 판정기다. 새 기사가 후보 클러스터들 중 "
    "'완전히 같은 하나의 구체적 사건'을 보도한 것과 같은지 판단한다.\n"
    "판단 기준(모두 실질적으로 일치해야 같은 사건): 주체(누가), 행동(무엇을 했나), "
    "대상·제품·프로젝트(무엇에 대해), 발생·발표 시점, 계약명·금액 등 식별 정보.\n"
    "규칙:\n"
    "1. 같은 회사·같은 산업·같은 키워드라는 이유만으로 합치지 않는다.\n"
    "2. 새 기사와 후보의 event_signature(주체·행동·대상·제품·시점·금액·식별자)가 "
    "하나라도 충돌하면 다른 사건이므로 new 로 판정한다.\n"
    "3. 같은 종목의 다른 발표, 같은 주제의 다른 후속 조치, 다른 시점의 사건은 서로 "
    "다른 사건이다.\n"
    "4. 조금이라도 애매하면 합치지 말고 new 로 판정한다.\n"
...
def build_user_prompt_v2(article: dict, candidates: list[dict]) -> str:
    ...
    lines.append(
        ...
        "event_signature 가 충돌하거나 같은 사건이 없으면 new 로 판정하라."
    )
    return "\n".join(lines)
